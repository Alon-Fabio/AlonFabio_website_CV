...
const App = () => {
  ...
  useEffect(() => {
    ...
    if (token) {
      setLoading(() => true);
      fetch("http://13.49.244.213:8080//signin", {
        method: "post",
        headers: {
  ...
  const fetchProfile = (token: string, id: number | null): void => {
    if (id !== null && id !== undefined) {
      fetch(`http://13.49.244.213:8080/profile/${id.toString()}`, {
        method: "get",
        headers: {
          "Content-Type": "application/json",
          Authentication: token,
  ...
  const onButtonSubmit = () => {
    if (input !== "") {
      setImageUrl(() => input);
      fetch("http://13.49.244.213:8080/imageurl", {
        method: "post",
        headers: {
          "Content-Type": "application/json",
          Authentication:
        ...
        .then((response) => {
          if (response) {
            fetch("http://13.49.244.213:8080/image", {
              method: "put",
              headers: {
                "Content-Type": "application/json",
                Authentication:
